local telescope_lazy_config = require("telescope._extensions.content.config")
local telescope_lazy_floating_window = require(
   "telescope._extensions.content.floating_window")
...
function M:open(path, resume_picker)
   ...
   if ret <= 0 then
      vim.notify(
         string.format("Failed to open terminal with cwd: '%s' (ret: '%d')", path,
            ret),
...
function M:set_keymaps(resume_picker)
   ...
   vim.keymap.set("t", telescope_lazy_config.opts.mappings.open_plugins_picker,
      function()
         self:close()
         resume_picker()
      end, opts)
