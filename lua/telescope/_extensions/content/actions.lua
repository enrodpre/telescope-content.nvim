...
local plugins_content_config = require("telescope._extensions.content.config")
local plugins_content_terminal = require(
"telescope._extensions.content.terminal")

local function warn_no_selection_action()
   vim.notify(
      "Please make a valid selection before performing the action.",
      vim.log.levels.WARN,
      { title = plugins_content_config.extension_name, }
   )
end
...
local function attach_mappings(_, map)
   map({ "n", "i", }, plugins_content_config.opts.mappings.open_plugins_picker,
      function()
         builtin.resume()
      end)
   return true
...
function M.change_cwd_to_plugin(prompt_bufnr)
   ...
   if ok then
      ...
      vim.notify(
         string.format(
            "Could not change cwd to: '%s'.\nError: '%s'" .. selected_entry.path,
            res),
   ...
   if plugins_content_config.opts.actions_opts.change_cwd_to_plugin.auto_close then
...
function M.open_in_terminal()
   ...
   local terminal = plugins_content_terminal.new()
...
function M.open_in_browser(prompt_bufnr)
   ...
   if not open_cmd then
      ...
      local ret = vim.fn.jobstart({ open_cmd, selected_entry.url, },
         { detach = true, })
      if ret <= 0 then
         vim.notify(
            string.format("Failed to open '%s'\nwith command: '%s' (ret: '%d')",
               selected_entry.url, open_cmd, ret),
            vim.log.levels.ERROR,
            { title = plugins_content_config.extension_name, }
         )
      end

      if plugins_content_config.opts.actions_opts.open_in_browser.auto_close then
         actions.close(prompt_bufnr)
      end
   end
end
